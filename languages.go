...
func workerLanguage(in chan ChItem, wg *sync.WaitGroup) {
	for {
		item, ok := <-in
		if !ok {
			break
		}
		*(item.Result) = getLanguage(item.Article)
	}
	wg.Done()
}

func getLanguages(articles []Article) []string {
	...
	for i := 0;i<workers;i++ {
		...
		go workerLanguage(inputs[len(inputs)-1], &wg)
	}
